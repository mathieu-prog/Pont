...
<?php 
if (isset($_POST['Update_Modelateur_btn'])) {
	// code...
...
if(isset($_POST['telephone']) and !empty($_POST['telephone']) and ($info['telephone']!= $_POST['telephone'])){
	$newPhone=htmlentities($_POST['telephone']);
...
if(isset($_POST['adresse']) and !empty($_POST['adresse']) and ($info['adresse']!= $_POST['adresse'])){
	...
	$modadresse=$pdo->prepare("update users set adresse=? where id_user=?");
	$modadresse->execute(array($newaddresse, $id));
	header("Location:manage_users.php");
